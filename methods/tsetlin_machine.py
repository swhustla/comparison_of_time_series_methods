...
def tsetlin_machine(
    ...
    combine_trend_seasonal_residual: Callable[
        [Prediction, Prediction, Prediction, Data], Prediction
    ],
    ...
    def predict(dataset: Dataset, parallel: bool = False) -> PredictionData:
        ...
        input_data_components = pd.concat(
            [trend_dataset_values, seasonal_dataset_values, residual_dataset_values],
            axis=1,
        )
        ...
        list_of_predictions = []
        for dataset_component in [
            trend_dataset_values,
            seasonal_dataset_values,
            residual_dataset_values,
        ]:
            ...
            list_of_predictions.append(
                get_forecast(dataset, params, number_of_iterations)
            )
