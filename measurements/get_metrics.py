...
def __get_root_mean_squared_error(
...
def __get_mean_absolute_percentage_error(
    ground_truth: np.ndarray, prediction: np.ndarray
) -> float:
...
__dict_of_metrics = {
    ...
    "mean_absolute_percentage_error": __get_mean_absolute_percentage_error,
}
...
def __metrics(prediction: PredictionData) -> Dict[str, float]:
    ...
    return dict(
        map(
            lambda metric: (
                metric,
                __round_to_2dp(__dict_of_metrics[metric](ground_truth, prediction)),
            ),
            __dict_of_metrics,
        )
    )
